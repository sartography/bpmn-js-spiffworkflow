...
import { TextFieldEntry, DescriptionEntry } from '@bpmn-io/properties-panel';
import {
...
export function MatchingCorrelationEntries(props) {
  const { idPrefix, translate, element, commandStack, moddle } = props;

  const correlationPropertyArray = findCorrelationPropertiesByMessage(element);

  const entries =
    correlationPropertyArray && correlationPropertyArray.length !== 0
      ? correlationPropertyArray.map(
      ...
      : [
          {
            ...
            value:
              'ℹ️ No matching conditions can be established since the selected message has no correlation properties',
            element,
            translate,
            commandStack,
          },
...
function MatchingConditionTextField(props) {
      ...
      if (extensionElements) {
        return extensionElements
          ...
          .filter(function getInstanceOfType(e) {
            return (
              e.$instanceOf('spiffworkflow:ProcessVariableCorrelation') &&
    ...
    if (trimmedValue === '' || !trimmedValue) {
      ...
      if (variableCorrelationObject) {
        const index = extensions
          .get('values')
          .indexOf(variableCorrelationObject);
        if (index > -1) {
      ...
      if (!variableCorrelationObject) {
        variableCorrelationObject = moddle.create(
  ...
  const getValue = () => {
    ...
    if (
      variableCorrelationObject &&
      variableCorrelationObject.propertyId ===
        correlationPropertyModdleElement.id
    ) {
