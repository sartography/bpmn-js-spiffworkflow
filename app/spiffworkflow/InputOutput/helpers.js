export function createSpecification(
  ...
  if (type === 'input') {
    ...
    if (!ioSpecification.inputSets.length) {
      ioSpecification.inputSets.push(
        bpmnFactory.create('bpmn:InputSet', { dataInputRefs: [newElement] })
      );
    } else {
    ...
    if (!ioSpecification.outputSets.length) {
      ioSpecification.outputSets.push(
        bpmnFactory.create('bpmn:OutputSet', { dataOutputRefs: [newElement] })
      );
    } else {
...
export function removeElementFromSpecification(element, entry, type) {
  ...
  const collection =
    type === 'input' ? ioSpecification.dataInputs : ioSpecification.dataOutputs;
  const setCollection =
    type === 'input' ? ioSpecification.inputSets : ioSpecification.outputSets;
  ...
  if (index > -1) {
    ...
    setCollection.forEach((set) => {
      const refIndex =
        set[type === 'input' ? 'dataInputRefs' : 'dataOutputRefs'].indexOf(
          removedElement
        );
      if (refIndex > -1) {
        set[type === 'input' ? 'dataInputRefs' : 'dataOutputRefs'].splice(
          refIndex,
          1
        );
      }
    ...
    console.error(
      `No ${type === 'input' ? 'DataInput' : 'DataOutput'} found for id ${
        entry.id
      }`
    );
  }
